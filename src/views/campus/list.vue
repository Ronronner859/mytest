...
const searchSuggestions = computed(() => {
  ...
  // 从职位数据中提取建议
  const jobsToSearch = category.value ? category.value.jobs : mockCampusCategories.flatMap(c => c.jobs);
...
const performSearch = () => {
  if (searchKeyword.value.trim()) {
    ...
    // 在路由更新后滚动到职位列表
    nextTick(() => {
      const jobListElement = document.querySelector('.page-title');
      if (jobListElement) {
        const offset = jobListElement.offsetTop - 20; // 减去一些偏移量，让标题显示在顶部
        window.scrollTo({
          top: offset,
          behavior: 'smooth'
        });
...
const category = computed(() => {
  return mockCampusCategories.find((c) => c.name === categoryName);
});
    ...
    .filter(job => {
      const matchLocation =
        ...
        (Array.isArray(job.location) 
          ? job.location.includes(filterLocation.value)
...
const latestJobs = computed(() => {
  ...
  const allJobs = mockCampusCategories.flatMap((c) => c.jobs);
...
const goToJobDetail = (jobId) => {
  ...
  router.push(`/campus-recruitment/campus/job/${jobId}`);
};
