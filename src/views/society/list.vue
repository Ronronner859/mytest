...
const locationCompanyMap = {
  '中山市大涌镇': '英诺莱比智能科技有限公司',
  '中山市火炬开发区': '英诺莱比精密制造有限公司',
  '中山市小榄镇': '英诺莱比金属表面处理有限公司'
};

const companyLocationMap = Object.entries(locationCompanyMap).reduce((acc, [location, company]) => {
...
watch(filterLocation, (newLocation) => {
  if (newLocation) {
    // 如果选择了地点，设置对应的公司
    filterCompany.value = locationCompanyMap[newLocation];
  } else {
    // 如果选择了"全部"，清除公司筛选
    filterCompany.value = '';
  }
...
// 监听公司变化
watch(filterCompany, (newCompany) => {
  if (newCompany) {
    // 如果选择了公司，设置对应的地点
    filterLocation.value = companyLocationMap[newCompany];
  } else {
    // 如果选择了"全部"，清除地点筛选
    filterLocation.value = '';
  }
...
const isLocationDisabled = (location) => {
  if (!filterLocation.value && !filterCompany.value) return false; // 如果都没选，则都不禁用
  if (location === '') return false; // "全部"选项永远不禁用
  
  if (filterCompany.value) {
    // 如果选择了公司，只启用对应的地点
    return location !== companyLocationMap[filterCompany.value];
  }
  
  if (filterLocation.value) {
    // 如果选择了地点，只禁用其他有映射关系的地点
    return location !== filterLocation.value && locationCompanyMap[location];
  }
  
  return false;
};

const isCompanyDisabled = (company) => {
  if (!filterLocation.value && !filterCompany.value) return false; // 如果都没选，则都不禁用
  if (company === '') return false; // "全部"选项永远不禁用
  
  if (filterLocation.value) {
    // 如果选择了地点，只启用对应的公司
    return company !== locationCompanyMap[filterLocation.value];
  }
  
  if (filterCompany.value) {
    // 如果选择了公司，只禁用其他有映射关系的公司
    return company !== filterCompany.value && companyLocationMap[company];
  }
...
// 获取地点对应的公司名称
const getCompanyForLocation = (location) => {
  return locationCompanyMap[location] || '';
};
        ...
        <van-collapse-item title="所属公司" name="company">
          <van-radio-group v-model="filterCompany">
            ...
            >
              ...
              >
                {{ opt.text }}
                <!-- <template v-if="getLocationForCompany(opt.value)">
                  <span class="company-location-tag">
                    {{ getLocationForCompany(opt.value) }}
                  </span>
                </template> -->
              </van-radio>
            </div>
